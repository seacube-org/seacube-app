  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" onFinish={onFinish}>
        <Form.Item name="label" label={i18n.t("address.label", { defaultValue: "名称" })}
...
export default function AddressesSettings() {
  ...
  useEffect(() => { reload(); }, [reload, activeOrgId]);
  ...
  const columns = useMemo(
    () => [
      { title: i18n.t("address.label", { defaultValue: "名称" }), dataIndex: "label", key: "label",
        render: (v: string) => v ? <Typography.Text strong>{v}</Typography.Text> : <Typography.Text type="secondary">—</Typography.Text> },
      { title: i18n.t("address.type", { defaultValue: "类型" }), key: "address_type", width: 120,
        render: (_: unknown, r: Address) => <Tag color={addressTypeColor(r.address_type)}>{addressTypeLabel(r.address_type)}</Tag> },
      { title: i18n.t("address.detail", { defaultValue: "地址" }), key: "detail",
        render: (_: unknown, r: Address) => formatAddressLine(r) || <Typography.Text type="secondary">—</Typography.Text> },
      { title: i18n.t("address.phone", { defaultValue: "电话" }), dataIndex: "phone", key: "phone", width: 140,
        render: (v: string) => v || <Typography.Text type="secondary">—</Typography.Text> },
      ...(isAdmin
        ? [{ title: i18n.t("access.actions", { defaultValue: "操作" }), key: "actions", width: 130,
            render: (_: unknown, r: Address) => (
              <Space size="small">
                <Button type="link" style={{ padding: 0 }} onClick={() => { setEditing(r); setDrawerOpen(true); }}>
