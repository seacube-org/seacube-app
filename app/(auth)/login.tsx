...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    if (!username || !password) { setError(i18n.t('auth.loginError.empty')); return; }
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput style={styles.input} placeholder={i18n.t('auth.username')} value={username} onChangeText={setUsername} autoCapitalize="none" />
      <TextInput style={styles.input} placeholder={i18n.t('auth.password')} value={password} onChangeText={setPassword} secureTextEntry />
      <TouchableOpacity style={styles.button} onPress={handleLogin} disabled={loading}>
        {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>{i18n.t('auth.loginButton')}</Text>}
      </TouchableOpacity>
    </View>
  );
}
