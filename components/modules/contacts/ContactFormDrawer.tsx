...
export default function ContactFormDrawer({ open, contact, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    form.resetFields();
    form.setFieldsValue({
      // `type` is derived server-side — not an editable field; shown as a title tag.
      name: contact?.name ?? "",
      email: contact?.email ?? "",
      phone: contact?.phone ?? "",
  ...
  const items = [
    ...
    },
    {
      ...
      forceRender: true,
      children: <PersonsTab schema={schema} />,
    },
    {
      key: "banks",
      label: i18n.t("contacts.tabBanks", { defaultValue: "银行账户" }),
      forceRender: true,
      children: <BankAccountsTab schema={schema} />,
    },
  ];
